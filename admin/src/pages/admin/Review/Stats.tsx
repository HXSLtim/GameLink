  ...
  const fetchAllData = async () => {
    ...
    try {
      const [statsRes, trendRes, playersRes, gamesRes] = await Promise.all([
        reviewStatsApi.getStats(),
        reviewStatsApi.getTrend({ days: 30 }),
        reviewStatsApi.getTopPlayers({ limit: 10, sortBy: rankType }),
        reviewStatsApi.getGameStats(),
      ]);

      if (statsRes.data.success) setStats(statsRes.data.data);
      if (trendRes.data.success) setTrend(trendRes.data.data || []);
      if (playersRes.data.success) setTopPlayers(playersRes.data.data || []);

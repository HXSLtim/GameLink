...
const SensitiveWords: React.FC = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editingWord) {
        // 编辑
        const response = await sensitiveWordApi.updateWord(editingWord.id, values);
        if (response.data.success) {
        ...
        // 新增
        const response = await sensitiveWordApi.addWord(values);
        if (response.data.success) {
  ...
  const handleDelete = async (id: number) => {
    try {
      const response = await sensitiveWordApi.deleteWord(id);
      if (response.data.success) {

...
const ReviewDetail: React.FC = () => {
  ...
  const fetchReview = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const response = await reviewApi.getReview(Number(id));
  ...
  const fetchLogs = useCallback(async () => {
    if (!id) return;
    try {
      const response = await reviewApi.getReviewLogs(Number(id));
      if (response.data.success) {
  ...
  const handleApprove = async () => {
    if (!id) return;
    try {
      const response = await reviewApi.approveReview(Number(id));
      if (response.data.success) {
  ...
  const handleReject = async () => {
    ...
    try {
      const response = await reviewApi.rejectReview(Number(id), rejectReason);
      if (response.data.success) {
  ...
  const handleDelete = async () => {
    if (!id) return;
    try {
      const response = await reviewApi.deleteReview(Number(id));
      if (response.data.success) {
  ...
  const handleReply = async () => {
    ...
    try {
      const response = await reviewReplyApi.createReply(Number(id), replyContent);
      if (response.data.success) {
  ...
  const handleDeleteReply = async (replyId: number) => {
    try {
      const response = await reviewReplyApi.deleteReply(replyId);
      if (response.data.success) {

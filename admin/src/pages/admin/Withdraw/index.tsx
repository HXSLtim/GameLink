...
import { exportToCSV, withdrawExportColumns } from '@/utils/export';
...
const WithdrawPage: React.FC = () => {
    ...
    const [selectedWithdrawIds, setSelectedWithdrawIds] = useState<number[]>([]);
    ...
    const loadData = useCallback(async (params?: WithdrawQueryParams) => {
        ...
        try {
            ...
            if (response.data.success) {
                const resData = response.data.data as { withdraws: Withdraw[]; total: number } | undefined;
                setWithdraws(resData?.withdraws || []);
    ...
    const loadStats = useCallback(async () => {
        try {
            ...
            setStats({
                pending: (pendingRes.data.data as { total?: number } | undefined)?.total || 0,
                approved: (approvedRes.data.data as { total?: number } | undefined)?.total || 0,
                completed: (completedRes.data.data as { total?: number } | undefined)?.total || 0,
                totalAmount: 0,
            });
        } catch (error) {
    ...
    const handleBatchApprove = (keys: React.Key[]) => {
        setSelectedWithdrawIds(keys ? keys.map(k => Number(k)) : []);
    ...
    const submitBatchApprove = async () => {
        try {
            ...
            let withdrawIds: number[] = [];

            if (values.target === 'selected') {
                withdrawIds = selectedWithdrawIds;
            } else {
                // 获取所有待审核的提现
                const response = await adminApi.getWithdraws({ status: 'pending', page_size: 1000 });
                if (response.data.success && response.data.data) {
                    const resData = response.data.data as { withdraws: Withdraw[] };
                    withdrawIds = resData.withdraws.map(w => w.id);
                }
            }

            if (withdrawIds.length === 0) {
                message.warning('没有符合条件的提现申请');
                return;
            }
            ...
            message.success(`批量批准完成，成功 ${successCount} 个`);
     ...
     * 批量拒绝
     */
    const handleBatchReject = (keys: React.Key[]) => {
        setSelectedWithdrawIds(keys ? keys.map(k => Number(k)) : []);
        batchForm.resetFields();
        batchForm.setFieldsValue({
            target: keys && keys.length > 0 ? 'selected' : 'all',
    ...
    const submitBatchReject = async () => {
        try {
            ...
            for (const id of withdrawIds) {
                try {
                    await adminApi.rejectWithdraw(id, { reason: values.reason || '批量拒绝' });
                    successCount++;
                } catch {
                    // 继续处理其他
                }
            }
            message.success(`批量拒绝完成，成功 ${successCount} 个`);
    ...
    const submitBatchComplete = async () => {
        try {
            ...
            if (values.target === 'selected') {
                ...
                const response = await adminApi.getWithdraws({ status: 'approved', page_size: 1000 });
                if (response.data.success && response.data.data) {
                    const resData = response.data.data as { withdraws: Withdraw[] };
                    withdrawIds = resData.withdraws.map(w => w.id);
                }
            ...
            for (const id of withdrawIds) {
                try {
                    await adminApi.completeWithdraw(id);
                    successCount++;
                } catch {
                    // 继续处理其他
                }
            }
            message.success(`批量完成打款，成功 ${successCount} 个`);
    ...
    const handleExport = async () => {
        try {
            ...
            const response = await adminApi.getWithdraws({ ...searchParams, page_size: 10000 });
            if (response.data.success && response.data.data) {
                const resData = response.data.data as { withdraws: Withdraw[] } | undefined;
                exportToCSV(resData?.withdraws as unknown as Record<string, unknown>[] || [], withdrawExportColumns, 'withdraws');
    ...
    const toolbarButtons: ToolbarButton[] = [
        {
            ...
            icon: <CheckOutlined />,
            needSelection: false,
            onClick: (keys) => handleBatchApprove(keys || []),
            permission: WITHDRAW_PERMISSIONS.APPROVE,
        },
        {
            text: '批量拒绝',
            icon: <CloseOutlined />,
            needSelection: false,
            danger: true,
            onClick: (keys) => handleBatchReject(keys || []),
            permission: WITHDRAW_PERMISSIONS.REJECT,
        },
        {
            text: '批量完成打款',
            icon: <DollarOutlined />,
        ...
        },
        {
            ...
            permission: WITHDRAW_PERMISSIONS.LIST,
        },
    ];
            ...
            >
                <Form form={batchForm} layout="vertical">
                    <Form.Item name="target" label="目标对象" rules={[{ required: true }]}>
                        <Radio.Group>
                            <Radio value="selected" disabled={selectedWithdrawIds.length === 0}>
                                选中的提现 {selectedWithdrawIds.length > 0 ? `(${selectedWithdrawIds.length})` : ''}
                            </Radio>
                            <Radio value="all">全部待审核提现</Radio>
                        </Radio.Group>
                    </Form.Item>
                </Form>
                ...
                <Form form={batchForm} layout="vertical">
                    <Form.Item name="target" label="目标对象" rules={[{ required: true }]}>
                        <Radio.Group>
                            ...
                            <Radio value="all">全部已批准待打款</Radio>
                        </Radio.Group>
                    </Form.Item>
                </Form>

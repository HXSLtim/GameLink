...
const AdminAlert: React.FC = () => {
  ...
  const loadAlerts = useCallback(async () => {
    ...
    try {
      ...
      if (res.data?.success) {
        ...
        setStats({
          highCount: alertData.filter((a: AlertType) => a.level === 'high').length,
          mediumCount: alertData.filter((a: AlertType) => a.level === 'medium').length,
          lowCount: alertData.filter((a: AlertType) => a.level === 'low').length,
          todayCount: alertData.filter((a: AlertType) => dayjs(a.createdAt).format('YYYY-MM-DD') === today).length,
        });
      } else {
  ...
  const handleViewDetail = (record: AlertType) => {
  ...
  const getLevelTag = (level: string) => {
    const config: Record<string, { color: string; icon: React.ReactNode; text: string }> = {
      ...
      low: { color: 'blue', icon: <InfoCircleOutlined />, text: '信息' },
  ...
  const alertColumns: ColumnsType<AlertType> = [
    ...
    { title: '时间', dataIndex: 'createdAt', key: 'createdAt', width: 170, render: (t) => dayjs(t).format('YYYY-MM-DD HH:mm:ss') },
    {
      ...
      key: 'isRead',
      width: 80,
      render: (isRead) => isRead ? <Tag color="default">已读</Tag> : <Badge status="processing" text={<Tag color="red">未读</Tag>} />,
    },
    {
      title: '操作',
      key: 'action',
      ...
      render: (_, record) => (
        <Space size={4}>
          ...
          {!record.isRead && (
            <Tooltip title="标记已读">
              <Button type="link" size="small" icon={<CheckOutlined />} onClick={() => handleMarkRead(String(record.id))}>已读</Button>
            </Tooltip>
          )}
        </Space>
      ),
  ...
  const ruleColumns: ColumnsType<AlertRule> = [
    ...
    { title: '持续时间', dataIndex: 'duration', key: 'duration', width: 100, render: (d) => `${d} 分钟` },
    {
      title: '通知渠道',
      dataIndex: 'notifyChannels',
      key: 'notifyChannels',
      width: 150,
      render: (channels) => <Space size={4} wrap>{channels.map((c: string) => <Tag key={c}>{c}</Tag>)}</Space>,
    },
    {
      title: '状态',
      dataIndex: 'isActive',
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space size={4}>
          ...
          <Popconfirm title="确定删除此规则？" onConfirm={() => handleDeleteRule(record.id)} okText="OK" cancelText="Cancel">
  ...
  const readRate = total > 0 ? Math.round(((total - unreadCount) / total) * 100) : 100;
    ...
    <div style={{ padding: 24 }}>
      ...
      {unreadCount > 0 && (
        <Alert
          ...
          title={
            <Space>
              <span>当前有 <Text strong style={{ color: token.colorError }}>{unreadCount}</Text> 条未读告警</span>
              {stats.highCount > 0 && <Tag color="red">严重 {stats.highCount}</Tag>}
              {stats.mediumCount > 0 && <Tag color="orange">警告 {stats.mediumCount}</Tag>}
            </Space>
          }
          action={
            <Button size="small" type="primary" onClick={handleBatchMarkRead} disabled={selectedRowKeys.length === 0}>
              批量标记已读
            </Button>
          }
        />
      ...
      <Row gutter={16} style={{ marginBottom: 16 }}>
        <Col xs={12} sm={6}>
          <Card hoverable style={{ minHeight: 120 }}>
            <Statistic title="告警总数" value={total} prefix={<BellOutlined />} />
          </Card>
        </Col>
        <Col xs={12} sm={6}>
          <Card hoverable style={{ minHeight: 120 }}>
            <Statistic
              title="未读告警"
              value={unreadCount}
              styles={{ content: { color: unreadCount > 0 ? token.colorError : token.colorSuccess } }}
              prefix={unreadCount > 0 ? <ExclamationCircleOutlined /> : <CheckCircleOutlined />}
            />
          </Card>
        </Col>
        <Col xs={12} sm={6}>
          <Card hoverable style={{ minHeight: 120 }}>
            <div style={{ marginBottom: 8 }}>
              <Text type="secondary">处理率</Text>
            </div>
            <Progress
              percent={readRate}
              status={readRate >= 80 ? 'success' : readRate >= 50 ? 'normal' : 'exception'}
              format={(p) => `${p}%`}
            />
          </Card>
        </Col>
      ...
      <Card>
        <Tabs
          ...
          items={[
            {
              key: 'alerts',
              label: <><BellOutlined /> 告警记录 <Badge count={unreadCount} size="small" /></>,
              children: (
                <>
                  {/* 筛选条件 */}
                  <Row gutter={16} style={{ marginBottom: 16 }}>
                    ...
                    <Col>
                      <Space>
                        <Button icon={<ReloadOutlined />} onClick={loadAlerts} loading={loading}>刷新</Button>
                        <Button
                          type="primary"
                          icon={<CheckOutlined />}
                        ...
                        >
                          批量已读 {selectedRowKeys.length > 0 && `(${selectedRowKeys.length})`}
                        </Button>
                      </Space>
                    </Col>
                  </Row>
            ...
            {
              key: 'rules',
              label: <><SettingOutlined /> 告警规则 <Badge count={rules.filter(r => r.isActive).length} color="green" size="small" /></>,
              children: (
                <>
                  <div style={{ marginBottom: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <Space>
                      ...
                      <Text type="secondary">
                        <InfoCircleOutlined /> 告警规则目前为本地配置，后端 API 开发中
                      </Text>
                    </Space>
                  </div>
        ...
        footer={[
          ...
          !selectedAlert?.isRead && (
            <Button
              ...
              onClick={() => {
                if (selectedAlert) {
                  handleMarkRead(String(selectedAlert.id));
        ...
        {selectedAlert && (
          <Descriptions column={2} bordered size="small">
            ...
            <Descriptions.Item label="状态">
              {selectedAlert.isRead ? <Tag color="default">已读</Tag> : <Tag color="red">未读</Tag>}
            </Descriptions.Item>
            <Descriptions.Item label="标题" span={2}>{selectedAlert.title}</Descriptions.Item>
            <Descriptions.Item label="类型">{getTypeTag(selectedAlert.type)}</Descriptions.Item>
            <Descriptions.Item label="级别">{getLevelTag(selectedAlert.level)}</Descriptions.Item>
            <Descriptions.Item label="来源">{selectedAlert.source || '-'}</Descriptions.Item>
            <Descriptions.Item label="时间">{dayjs(selectedAlert.createdAt).format('YYYY-MM-DD HH:mm:ss')}</Descriptions.Item>

# 🚀 GameLink 9周完整开发计划

## 📊 当前状态
- **功能完整度**: 75%
- **测试覆盖率**: 65.66%
- **项目评级**: B+ (良好)

## 🎯 目标状态
- **功能完整度**: 95%
- **测试覆盖率**: 80%+
- **项目评级**: A (优秀)

---

## 📅 Phase 1: 测试覆盖率提升 (Week 1)

### 🎯 目标
- 测试覆盖率: 65.66% → 80%+
- Handler层: 43% → 70%+
- Service层: 72% → 85%+

### 📋 任务清单

#### Day 1-2: Admin Handler测试
```bash
✅ ranking_test.go - 排行榜管理测试
⬜ 补充commission_test.go完整场景
⬜ 补充withdraw_test.go完整场景
⬜ 补充item_test.go完整场景
⬜ 创建stats完整测试
⬜ 创建system完整测试
```

**预期成果**: admin handler 32.6% → 60%+

#### Day 3-4: User Handler测试
```bash
⬜ 扩展order_test.go
   - 订单创建边界条件
   - 订单取消各种状态
   - 订单完成流程
   - 订单列表筛选
⬜ 扩展payment_test.go
   - 支付创建
   - 支付回调处理
   - 支付失败场景
   - 退款流程
⬜ 扩展review_test.go
   - 评价创建
   - 评价编辑
   - 评价删除
   - 评价查询
⬜ 扩展player_test.go
   - 玩家查询
   - 玩家筛选
   - 游戏筛选
```

**预期成果**: user handler 39.3% → 65%+

#### Day 5-6: Player Handler测试
```bash
⬜ 扩展order_test.go
   - 接单流程
   - 拒单场景
   - 开始服务
   - 完成服务
⬜ 扩展earnings_test.go
   - 收益查询
   - 收益统计
   - 提现申请
   - 提现记录
⬜ 扩展profile_test.go
   - 资料更新
   - 认证申请
   - 状态管理
⬜ 创建gift_test.go
   - 礼物接收
   - 礼物统计
```

**预期成果**: player handler 39.1% → 65%+

#### Day 7: Service层测试补充
```bash
⬜ commission_test.go
   - 三层抽成计算
   - 边界条件测试
   - 月度结算
⬜ role_test.go
   - 权限继承
   - 角色冲突检测
⬜ admin_test.go
   - 批量操作
   - 审计日志
```

**预期成果**: Service层 72% → 85%+

### ✅ 验收标准
- [ ] 总体覆盖率 ≥ 80%
- [ ] Handler层平均 ≥ 65%
- [ ] Service层平均 ≥ 85%
- [ ] 所有测试通过
- [ ] 无编译错误

---

## 🔒 Phase 2: 安全增强 (Week 2)

### 🎯 目标
- 实现CSRF保护
- 实现API限流
- 实现文件上传安全
- 加强输入验证

### 📋 任务清单

#### Day 1-2: CSRF保护
```go
// 文件: internal/middleware/csrf.go
⬜ 实现CSRF token生成
⬜ 实现CSRF验证中间件
⬜ 集成到路由
⬜ 编写测试用例
```

**代码结构**:
```go
package middleware

type CSRFConfig struct {
    TokenLength int
    CookieName  string
    HeaderName  string
    Secret      string
}

func CSRF(config CSRFConfig) gin.HandlerFunc {
    // 实现CSRF保护
}
```

#### Day 3-4: API限流
```go
// 文件: internal/middleware/ratelimit.go
⬜ 实现基于IP的限流
⬜ 实现基于用户的限流
⬜ 实现滑动窗口算法
⬜ Redis存储限流数据
⬜ 编写测试用例
```

**代码结构**:
```go
package middleware

type RateLimitConfig struct {
    RequestsPerMinute int
    BurstSize         int
    KeyFunc           func(*gin.Context) string
}

func RateLimit(config RateLimitConfig) gin.HandlerFunc {
    // 实现限流
}
```

#### Day 5-7: 文件上传安全
```go
// 文件: internal/handler/upload/upload.go
⬜ 实现文件上传handler
⬜ 文件类型验证 (白名单)
⬜ 文件大小限制
⬜ 文件名随机化
⬜ 病毒扫描集成 (可选)
⬜ 存储到OSS (阿里云/七牛)
⬜ 编写测试用例
```

**数据库设计**:
```sql
CREATE TABLE uploads (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    user_id BIGINT NOT NULL,
    file_name VARCHAR(255) NOT NULL,
    file_path VARCHAR(500) NOT NULL,
    file_size BIGINT NOT NULL,
    mime_type VARCHAR(100) NOT NULL,
    upload_type VARCHAR(50) NOT NULL, -- avatar, certification, etc
    status VARCHAR(20) DEFAULT 'pending',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    INDEX idx_user_id (user_id),
    INDEX idx_upload_type (upload_type)
);
```

### ✅ 验收标准
- [ ] CSRF保护正常工作
- [ ] API限流有效防止刷接口
- [ ] 文件上传安全可靠
- [ ] 所有安全测试通过
- [ ] 安全文档完善

---

## 💎 Phase 3: 会员订阅系统 (Week 3-4)

### 🎯 目标
- 实现用户VIP会员
- 实现陪玩师认证会员
- 实现自动续费
- 实现会员权益管理

### 📋 任务清单

#### Week 3 Day 1-3: 数据库设计与Model
```sql
-- 会员等级表
CREATE TABLE membership_tiers (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    name VARCHAR(50) NOT NULL,
    type VARCHAR(20) NOT NULL, -- user_vip, player_certified
    price_cents INT NOT NULL,
    duration_days INT NOT NULL,
    benefits JSON NOT NULL,
    sort_order INT DEFAULT 0,
    is_active BOOLEAN DEFAULT TRUE,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
);

-- 会员订阅表
CREATE TABLE memberships (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    user_id BIGINT NOT NULL,
    tier_id BIGINT NOT NULL,
    start_date TIMESTAMP NOT NULL,
    end_date TIMESTAMP NOT NULL,
    status VARCHAR(20) DEFAULT 'active', -- active, expired, cancelled
    auto_renew BOOLEAN DEFAULT FALSE,
    payment_method VARCHAR(50),
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    INDEX idx_user_id (user_id),
    INDEX idx_status (status),
    INDEX idx_end_date (end_date)
);

-- 会员权益表
CREATE TABLE membership_benefits (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    tier_id BIGINT NOT NULL,
    benefit_key VARCHAR(100) NOT NULL,
    benefit_value VARCHAR(500) NOT NULL,
    description TEXT,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    INDEX idx_tier_id (tier_id)
);
```

**Model定义**:
```go
// internal/model/membership.go
⬜ MembershipTier model
⬜ Membership model
⬜ MembershipBenefit model
```

#### Week 3 Day 4-7: Repository层
```go
// internal/repository/membership/membership.go
⬜ 实现MembershipRepository接口
⬜ CreateTier - 创建会员等级
⬜ GetTier - 获取会员等级
⬜ ListTiers - 列出会员等级
⬜ UpdateTier - 更新会员等级
⬜ CreateMembership - 创建会员订阅
⬜ GetUserMembership - 获取用户会员
⬜ UpdateMembership - 更新会员状态
⬜ ListExpiringMemberships - 获取即将过期的会员
⬜ 编写完整测试 (目标覆盖率90%+)
```

#### Week 4 Day 1-4: Service层
```go
// internal/service/membership/membership.go
⬜ 实现MembershipService
⬜ Subscribe - 订阅会员
⬜ Renew - 续费会员
⬜ Cancel - 取消会员
⬜ CheckBenefit - 检查权益
⬜ AutoRenewTask - 自动续费任务
⬜ ExpireTask - 过期处理任务
⬜ 编写完整测试 (目标覆盖率85%+)
```

**业务逻辑**:
```go
// 订阅流程
1. 选择会员等级
2. 创建支付订单
3. 支付成功回调
4. 激活会员
5. 发送通知

// 自动续费流程
1. 定时任务扫描即将过期的会员
2. 检查auto_renew标志
3. 创建续费订单
4. 扣款
5. 延长会员期限
```

#### Week 4 Day 5-7: Handler层与前端API
```go
// internal/handler/user/membership.go
⬜ ListTiers - 获取会员等级列表
⬜ Subscribe - 订阅会员
⬜ GetMyMembership - 获取我的会员
⬜ CancelAutoRenew - 取消自动续费
⬜ EnableAutoRenew - 开启自动续费

// internal/handler/admin/membership.go
⬜ CreateTier - 创建会员等级
⬜ UpdateTier - 更新会员等级
⬜ ListMemberships - 查看所有会员
⬜ GetMembershipStats - 会员统计
⬜ 编写完整测试
```

### ✅ 验收标准
- [ ] 数据库表创建完成
- [ ] Repository层测试覆盖率 ≥ 90%
- [ ] Service层测试覆盖率 ≥ 85%
- [ ] Handler层测试覆盖率 ≥ 70%
- [ ] 订阅流程完整可用
- [ ] 自动续费功能正常
- [ ] API文档完善

---

## 💬 Phase 4: 实时聊天功能 (Week 5-6)

### 🎯 目标
- 实现WebSocket服务
- 实现消息存储
- 实现离线消息
- 实现消息推送

### 📋 任务清单

#### Week 5 Day 1-3: WebSocket服务
```go
// internal/websocket/hub.go
⬜ 实现WebSocket Hub
⬜ 客户端连接管理
⬜ 消息广播
⬜ 心跳检测
⬜ 断线重连

// internal/websocket/client.go
⬜ 实现Client结构
⬜ 消息发送
⬜ 消息接收
⬜ 连接状态管理
```

**技术选型**:
```go
// 使用gorilla/websocket
import "github.com/gorilla/websocket"

type Hub struct {
    clients    map[uint64]*Client
    broadcast  chan *Message
    register   chan *Client
    unregister chan *Client
}

type Client struct {
    hub    *Hub
    conn   *websocket.Conn
    userID uint64
    send   chan []byte
}
```

#### Week 5 Day 4-7: 消息存储
```sql
-- 聊天消息表
CREATE TABLE chat_messages (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    conversation_id BIGINT NOT NULL,
    sender_id BIGINT NOT NULL,
    receiver_id BIGINT NOT NULL,
    message_type VARCHAR(20) DEFAULT 'text', -- text, image, voice
    content TEXT NOT NULL,
    is_read BOOLEAN DEFAULT FALSE,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    INDEX idx_conversation (conversation_id),
    INDEX idx_sender (sender_id),
    INDEX idx_receiver (receiver_id),
    INDEX idx_created (created_at)
);

-- 会话表
CREATE TABLE conversations (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    order_id BIGINT,
    user_id BIGINT NOT NULL,
    player_id BIGINT NOT NULL,
    last_message TEXT,
    last_message_at TIMESTAMP,
    unread_count INT DEFAULT 0,
    status VARCHAR(20) DEFAULT 'active',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    INDEX idx_user_id (user_id),
    INDEX idx_player_id (player_id),
    INDEX idx_order_id (order_id)
);
```

```go
// internal/repository/chat/chat.go
⬜ 实现ChatRepository
⬜ CreateMessage - 创建消息
⬜ GetMessages - 获取消息列表
⬜ MarkAsRead - 标记已读
⬜ GetConversation - 获取会话
⬜ ListConversations - 获取会话列表
⬜ UpdateConversation - 更新会话
```

#### Week 6 Day 1-4: Service层与Handler
```go
// internal/service/chat/chat.go
⬜ 实现ChatService
⬜ SendMessage - 发送消息
⬜ GetHistory - 获取历史消息
⬜ GetConversations - 获取会话列表
⬜ MarkRead - 标记已读
⬜ 离线消息处理

// internal/handler/chat/chat.go
⬜ WebSocket连接endpoint
⬜ 获取历史消息API
⬜ 获取会话列表API
⬜ 标记已读API
```

#### Week 6 Day 5-7: 消息推送与测试
```go
⬜ 集成推送服务 (极光推送/Firebase)
⬜ 离线消息推送
⬜ 消息提醒
⬜ 编写完整测试
⬜ 性能测试 (支持1000+并发连接)
```

### ✅ 验收标准
- [ ] WebSocket服务稳定运行
- [ ] 消息实时收发正常
- [ ] 离线消息正常存储和推送
- [ ] 支持1000+并发连接
- [ ] 消息不丢失
- [ ] 测试覆盖率 ≥ 75%

---

## 🎁 Phase 5: 营销工具系统 (Week 7-8)

### 🎯 目标
- 实现优惠券系统
- 实现活动配置
- 实现推送通知
- 实现营销数据统计

### 📋 任务清单

#### Week 7 Day 1-4: 优惠券系统
```sql
-- 优惠券模板表
CREATE TABLE coupon_templates (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    name VARCHAR(100) NOT NULL,
    type VARCHAR(20) NOT NULL, -- discount, cashback, gift
    discount_type VARCHAR(20), -- percentage, fixed
    discount_value INT NOT NULL,
    min_order_amount INT DEFAULT 0,
    max_discount_amount INT,
    total_quantity INT NOT NULL,
    remaining_quantity INT NOT NULL,
    valid_from TIMESTAMP NOT NULL,
    valid_to TIMESTAMP NOT NULL,
    applicable_games JSON,
    applicable_players JSON,
    status VARCHAR(20) DEFAULT 'active',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    INDEX idx_status (status),
    INDEX idx_valid_period (valid_from, valid_to)
);

-- 用户优惠券表
CREATE TABLE user_coupons (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    user_id BIGINT NOT NULL,
    template_id BIGINT NOT NULL,
    coupon_code VARCHAR(50) UNIQUE NOT NULL,
    status VARCHAR(20) DEFAULT 'unused', -- unused, used, expired
    used_at TIMESTAMP,
    order_id BIGINT,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    INDEX idx_user_id (user_id),
    INDEX idx_coupon_code (coupon_code),
    INDEX idx_status (status)
);
```

```go
// internal/service/coupon/coupon.go
⬜ 实现CouponService
⬜ CreateTemplate - 创建优惠券模板
⬜ IssueCoupon - 发放优惠券
⬜ UseCoupon - 使用优惠券
⬜ ValidateCoupon - 验证优惠券
⬜ GetUserCoupons - 获取用户优惠券
⬜ CalculateDiscount - 计算折扣
```

#### Week 7 Day 5-7: 活动配置
```sql
-- 营销活动表
CREATE TABLE marketing_campaigns (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    name VARCHAR(100) NOT NULL,
    type VARCHAR(50) NOT NULL, -- new_user, recharge, invite
    description TEXT,
    rules JSON NOT NULL,
    rewards JSON NOT NULL,
    start_time TIMESTAMP NOT NULL,
    end_time TIMESTAMP NOT NULL,
    status VARCHAR(20) DEFAULT 'draft',
    created_by BIGINT NOT NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    INDEX idx_type (type),
    INDEX idx_status (status),
    INDEX idx_time_range (start_time, end_time)
);

-- 活动参与记录
CREATE TABLE campaign_participations (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    campaign_id BIGINT NOT NULL,
    user_id BIGINT NOT NULL,
    reward_type VARCHAR(50),
    reward_value VARCHAR(200),
    status VARCHAR(20) DEFAULT 'pending',
    completed_at TIMESTAMP,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    INDEX idx_campaign_id (campaign_id),
    INDEX idx_user_id (user_id)
);
```

```go
// internal/service/campaign/campaign.go
⬜ 实现CampaignService
⬜ CreateCampaign - 创建活动
⬜ ParticipateInCampaign - 参与活动
⬜ CheckEligibility - 检查资格
⬜ DistributeReward - 发放奖励
⬜ GetCampaignStats - 活动统计
```

#### Week 8 Day 1-4: 推送通知
```sql
-- 通知模板表
CREATE TABLE notification_templates (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    name VARCHAR(100) NOT NULL,
    type VARCHAR(50) NOT NULL, -- order, payment, system
    title VARCHAR(200) NOT NULL,
    content TEXT NOT NULL,
    variables JSON,
    channels JSON, -- push, sms, email
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

-- 通知记录表
CREATE TABLE notifications (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    user_id BIGINT NOT NULL,
    template_id BIGINT,
    title VARCHAR(200) NOT NULL,
    content TEXT NOT NULL,
    type VARCHAR(50) NOT NULL,
    channel VARCHAR(20) NOT NULL,
    status VARCHAR(20) DEFAULT 'pending',
    sent_at TIMESTAMP,
    read_at TIMESTAMP,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    INDEX idx_user_id (user_id),
    INDEX idx_status (status),
    INDEX idx_type (type)
);
```

```go
// internal/service/notification/notification.go
⬜ 实现NotificationService
⬜ SendNotification - 发送通知
⬜ SendBulkNotification - 批量发送
⬜ GetUserNotifications - 获取用户通知
⬜ MarkAsRead - 标记已读
⬜ 集成第三方推送服务
```

#### Week 8 Day 5-7: Handler与测试
```go
// internal/handler/admin/marketing.go
⬜ 优惠券管理API
⬜ 活动管理API
⬜ 通知管理API
⬜ 营销数据统计API

// internal/handler/user/marketing.go
⬜ 获取可用优惠券
⬜ 领取优惠券
⬜ 参与活动
⬜ 获取通知列表

⬜ 编写完整测试
⬜ 集成测试
```

### ✅ 验收标准
- [ ] 优惠券系统完整可用
- [ ] 活动配置灵活
- [ ] 推送通知稳定
- [ ] 营销数据统计准确
- [ ] 测试覆盖率 ≥ 75%

---

## ⚡ Phase 6: 性能优化与监控 (Week 9)

### 🎯 目标
- 实现Redis缓存策略
- 数据库查询优化
- 实现监控告警
- 性能测试

### 📋 任务清单

#### Day 1-3: 缓存优化
```go
// internal/cache/strategy.go
⬜ 玩家列表缓存 (5分钟)
⬜ 游戏列表缓存 (30分钟)
⬜ 用户信息缓存 (10分钟)
⬜ 热门数据缓存
⬜ 缓存预热
⬜ 缓存穿透防护
⬜ 缓存雪崩防护
```

**缓存策略**:
```go
// 多级缓存
1. 本地缓存 (go-cache) - 热点数据
2. Redis缓存 - 共享数据
3. 数据库 - 持久化数据

// 缓存更新策略
1. 主动更新 - 数据变更时
2. 被动更新 - 缓存过期时
3. 定时更新 - 定时任务
```

#### Day 4-5: 数据库优化
```sql
⬜ 添加必要的索引
⬜ 优化慢查询
⬜ 分页查询优化
⬜ 连接池配置
⬜ 读写分离准备
```

**优化重点**:
```sql
-- 添加复合索引
CREATE INDEX idx_orders_user_status ON orders(user_id, status, created_at);
CREATE INDEX idx_players_game_status ON players(game_id, status, rating);

-- 优化统计查询
-- 使用物化视图或定时统计表
```

#### Day 6-7: 监控与告警
```go
// internal/metrics/metrics.go
⬜ 实现Prometheus指标收集
⬜ API响应时间监控
⬜ 错误率监控
⬜ 数据库连接池监控
⬜ Redis连接监控
⬜ WebSocket连接数监控

// internal/alert/alert.go
⬜ 实现告警规则
⬜ 邮件告警
⬜ 钉钉/企业微信告警
⬜ 告警聚合
```

**监控指标**:
```
1. 系统指标
   - CPU使用率
   - 内存使用率
   - 磁盘使用率
   
2. 应用指标
   - QPS
   - 响应时间
   - 错误率
   - 并发连接数
   
3. 业务指标
   - 订单量
   - 支付成功率
   - 用户活跃度
```

### ✅ 验收标准
- [ ] 缓存命中率 ≥ 80%
- [ ] API平均响应时间 < 200ms
- [ ] 数据库慢查询优化完成
- [ ] 监控系统正常运行
- [ ] 告警规则配置完成

---

## 📊 总体验收标准

### 功能完整性
- [ ] 三端功能完整度 ≥ 95%
- [ ] 核心业务流程完整
- [ ] 所有API文档完善

### 代码质量
- [ ] 测试覆盖率 ≥ 80%
- [ ] 所有测试通过
- [ ] 代码规范检查通过
- [ ] 无严重安全漏洞

### 性能指标
- [ ] API响应时间 < 200ms (P95)
- [ ] 支持1000+并发用户
- [ ] 数据库查询优化完成
- [ ] 缓存策略有效

### 安全性
- [ ] CSRF保护完成
- [ ] API限流完成
- [ ] 文件上传安全
- [ ] 所有输入验证

### 文档
- [ ] API文档完整
- [ ] 部署文档完整
- [ ] 运维文档完整
- [ ] 开发文档完整

---

## 🎯 最终目标

**项目评级**: B+ → A
**功能完整度**: 75% → 95%
**测试覆盖率**: 65.66% → 80%+
**可上线状态**: ✅ 完全就绪

---

## 💡 开发建议

1. **每天提交代码** - 保持小步快跑
2. **先写测试** - TDD开发模式
3. **及时文档** - 代码即文档
4. **代码审查** - 保证质量
5. **持续集成** - 自动化测试

---

## 📞 需要支持

如需要我帮助：
1. ✅ 创建具体的代码文件
2. ✅ 实现某个功能模块
3. ✅ 编写测试用例
4. ✅ 优化性能
5. ✅ 解决技术问题

随时告诉我，我会立即开始！

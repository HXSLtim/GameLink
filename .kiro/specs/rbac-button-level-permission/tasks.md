...
- [x] 25. 种子数据更新
  ...
  - [x] 25.3 标记系统权限和角色
    - IsSystem = true
    - 创建 seedSystemPermissions 函数从 GetAllPermissionDefinitions 种子化权限
    - 所有系统权限自动设置 IsSystem = true
    - 系统角色在 SystemRoleDefinitions 中已设置 IsSystem = true
    - _Requirements: 1.5_
  - [x] 25.4 确保种子数据幂等执行
    - seedSystemPermissions 支持幂等执行（按 code 或 method+path 查找更新）
    - seedDefaultRoles 支持幂等执行（按 slug 查找更新）
    - 测试验证多次执行不会产生重复数据
    - _Requirements: 1.1_

            ...
            <div className="max-w-6xl mx-auto h-full flex flex-col space-y-6">
                ...
                <Tabs defaultValue="all" value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col">
                    <TabsList className="grid w-full grid-cols-4 md:w-[400px] p-1 bg-muted/40 backdrop-blur-md border border-white/5 rounded-full">
                        ...
                        <TabsTrigger value="active" className="rounded-full data-[state=active]:bg-background/80 transition-all">{t('order.tab_active')}</TabsTrigger>
                        <TabsTrigger value="completed" className="rounded-full data-[state=active]:bg-background/80 transition-all">{t('order.tab_done')}</TabsTrigger>
                        <TabsTrigger value="cancelled" className="rounded-full data-[state=active]:bg-background/80 transition-all">{t('order.tab_cancel')}</TabsTrigger>
                    </TabsList>
                    ...
                    <div className="mt-6 flex-1 min-h-0">
                        {filteredOrders.length === 0 && !loading ? (
                            <div className="flex flex-col items-center justify-center py-20 text-center animate-in fade-in zoom-in-95">
                                ...
                                <p className="text-sm text-muted-foreground mt-1 max-w-[250px] mx-auto">{t('order.empty_desc')}</p>
                                            ...
                                            >
                                                ...
                                                <CardContent className="p-5 flex-1">
                                                    <div className="flex items-start gap-4">
                                                        ...
                                                        <div className="flex-1 min-w-0 space-y-2">
                                                            ...
                                                            <Badge variant="secondary" className="text-[10px] font-normal px-1.5 py-0 h-5">{t('order.service_type')}</Badge>
                                                ...
                                                <CardFooter className="bg-muted/5 p-3 px-5 flex justify-between gap-3 border-t border-white/5 mt-auto">
                                                    <Button variant="ghost" size="sm" className="h-8 hover:bg-white/5 text-xs" onClick={() => navigate(`/orders/${order.id}`)}>
                                                        {t('order.details')} <ChevronRight className="ml-1 h-3 w-3" />
                                                    </Button>

...
func main() {
	fmt.Println("GameLink Service层测试文件合并工具")
	fmt.Println("====================================")
...
func mergeModule(dir string, files []string, output string) error {
	...
	if len(imports) > 0 {
		...
		// 标准库
		var stdLibs, extLibs []string
		for imp := range imports {
			if strings.Contains(imp, ".") {
				extLibs = append(extLibs, imp)
			} else {
...
func parseGoFile(content string) (packageName string, imports map[string]bool, allCode []string, err error) {
	imports = make(map[string]bool)
	...
	for scanner.Scan() {
		...
		if !inCodeBlock {
			// Mock结构体
			if strings.HasPrefix(trimmed, "type Mock") && strings.HasSuffix(trimmed, "struct {") {
				inCodeBlock = true

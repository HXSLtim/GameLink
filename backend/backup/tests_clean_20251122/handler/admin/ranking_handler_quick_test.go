...
type fakeRankingCfgRepo struct {
	store map[uint64]model.RankingCommissionConfig
	next  uint64
}

func newFakeRankingCfgRepo() *fakeRankingCfgRepo {
	return &fakeRankingCfgRepo{store: map[uint64]model.RankingCommissionConfig{}, next: 1}
}
func (f *fakeRankingCfgRepo) CreateConfig(_ context.Context, c *model.RankingCommissionConfig) error {
	c.ID = f.next
	f.next++
	f.store[c.ID] = *c
	return nil
}
func (f *fakeRankingCfgRepo) GetConfig(_ context.Context, id uint64) (*model.RankingCommissionConfig, error) {
	v, ok := f.store[id]
	if !ok {
		return nil, rankingrepo.ErrNotFound
	}
	c := v
	return &c, nil
}
func (f *fakeRankingCfgRepo) GetActiveConfigForMonth(_ context.Context, _ model.RankingType, _ string) (*model.RankingCommissionConfig, error) {
	return nil, rankingrepo.ErrNotFound
}
func (f *fakeRankingCfgRepo) ListConfigs(_ context.Context, _ rankingrepo.RankingCommissionConfigListOptions) ([]model.RankingCommissionConfig, int64, error) {
	out := make([]model.RankingCommissionConfig, 0, len(f.store))
	for _, v := range f.store {
		out = append(out, v)
	}
	return out, int64(len(out)), nil
}
func (f *fakeRankingCfgRepo) UpdateConfig(_ context.Context, c *model.RankingCommissionConfig) error {
	f.store[c.ID] = *c
	return nil
}
func (f *fakeRankingCfgRepo) DeleteConfig(_ context.Context, id uint64) error {
	delete(f.store, id)
	return nil
}

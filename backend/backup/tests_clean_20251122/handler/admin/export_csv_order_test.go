...
func TestExportOperationLogsCSV_Order_EN_NoBOM_TZ(t *testing.T) {
	...
	if cd := w.Header().Get("Content-Disposition"); !strings.Contains(cd, "attachment") || !strings.Contains(cd, "order_9_logs.csv") {
		t.Fatalf("unexpected content disposition: %q", cd)
	}
	body := w.Body.String()
	if strings.HasPrefix(body, "\uFEFF") {
		t.Fatalf("unexpected BOM present")
	}
	if !strings.Contains(body, "created_at") || !strings.Contains(body, "metadata") {
		t.Fatalf("expected headers present")
	}
	_ = body
}

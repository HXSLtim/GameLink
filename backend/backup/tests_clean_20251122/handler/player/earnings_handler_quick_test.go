...
func (fakePlayerRepoEarn) ListPaged(ctx context.Context, page int, pageSize int) ([]model.Player, int64, error) {
	return []model.Player{{Base: model.Base{ID: 1}, UserID: 1, Nickname: "p"}}, 1, nil
}
func (fakePlayerRepoEarn) Get(ctx context.Context, id uint64) (*model.Player, error) {
...
func (fakePlayerRepoEarn) GetByUserID(ctx context.Context, userID uint64) (*model.Player, error) {
	return &model.Player{Base: model.Base{ID: 1}, UserID: userID, Nickname: "p"}, nil
}
...
func (fakeOrderRepoEarn) Get(ctx context.Context, id uint64) (*model.Order, error) {
...
type fakeWithdrawRepoEarn struct {
	balance *withdrawrepo.PlayerBalance
	list    []model.Withdraw
}

func (f *fakeWithdrawRepoEarn) Create(ctx context.Context, w *model.Withdraw) error {
	w.ID = 1
	f.list = append(f.list, *w)
	return nil
}
func (f *fakeWithdrawRepoEarn) Get(ctx context.Context, id uint64) (*model.Withdraw, error) {
	return nil, repository.ErrNotFound
}

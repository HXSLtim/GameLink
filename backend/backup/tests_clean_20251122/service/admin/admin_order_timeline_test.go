...
func (t timelineOrders) List(context.Context, repository.OrderListOptions) ([]model.Order, int64, error) {
	return nil, 0, nil
}
...
func (p timelinePayments) List(context.Context, repository.PaymentListOptions) ([]model.Payment, int64, error) {
	return p.items, int64(len(p.items)), nil
}
func (p timelinePayments) Get(context.Context, uint64) (*model.Payment, error) {
...
func (l timelineOpLogs) ListByEntity(context.Context, string, uint64, repository.OperationLogListOptions) ([]model.OperationLog, int64, error) {
	return l.logs, int64(len(l.logs)), nil
}
...
func (txReject) WithTx(context.Context, func(r *common.Repos) error) error {
...
func TestGetOrderRefunds_SummaryAddedWhenNoPaymentMatches(t *testing.T) {
	...
	for _, it := range items {
		if it.AmountCents == 300 {
			hasSummary = true
		}
	}
	if !hasSummary {
		t.Fatal("expected summary refund item")
	}
}
...
func TestGetOrderRefunds_NoSummaryWhenPaymentMatches(t *testing.T) {
	...
	count := 0
	for _, it := range items {
		if it.AmountCents == 500 {
			count++
		}
	}
	if count != 1 {
		t.Fatalf("expected only payment-based refund item, got %d", count)
	}
}

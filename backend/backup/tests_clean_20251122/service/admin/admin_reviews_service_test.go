...
func (reviewsPagerRepo) List(ctx context.Context, opts repository.ReviewListOptions) ([]model.Review, int64, error) {
	_ = ctx
	if opts.Page == 1 {
		return []model.Review{{Base: model.Base{ID: 1}}}, 201, nil
	}
	if opts.Page == 2 {
		return []model.Review{{Base: model.Base{ID: 2}}}, 201, nil
	}
	return []model.Review{}, 201, nil
}
func (reviewsPagerRepo) Get(context.Context, uint64) (*model.Review, error) {
...
func (t *txReviewsPager) WithTx(ctx context.Context, fn func(r *common.Repos) error) error {
	return fn(&t.repos)
}
...
func TestGetOrderReviews_AggregatesPages(t *testing.T) {
	...
	if len(items) != 2 {
		t.Fatalf("expected 2 reviews, got %d", len(items))
	}
}

# 批量发送通知API测试脚本 (PowerShell版本)

# 配置
$API_URL = "http://localhost:8080/api/v1/admin/users/batch/notification"
# 请替换为你的实际token
$TOKEN = "YOUR_ADMIN_TOKEN_HERE"

$headers = @{
    "Authorization" = "Bearer $TOKEN"
    "Content-Type" = "application/json"
}

Write-Host "========================================" -ForegroundColor Cyan
Write-Host "批量发送通知API测试脚本" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""

# 测试1: 指定用户列表模式
Write-Host "测试1: 为指定用户发送通知" -ForegroundColor Green
Write-Host "目标: 用户ID 1,2,3"
$body1 = @{
    target = "users"
    userIds = @(1, 2, 3)
    title = "测试-系统通知"
    content = "这是一条测试系统通知消息"
    type = "system"
} | ConvertTo-Json

try {
    $response = Invoke-RestMethod -Uri $API_URL -Method Post -Headers $headers -Body $body1
    $response | ConvertTo-Json -Depth 10 | Write-Host
} catch {
    Write-Host "错误: $_" -ForegroundColor Red
}

Write-Host ""
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""

# 测试2: 按角色筛选模式 - 单个角色
Write-Host "测试2: 为所有player角色发送通知" -ForegroundColor Green
Write-Host "角色: player"
$body2 = @{
    target = "role"
    roles = @("player")
    title = "测试-陪玩师培训通知"
    content = "本周五举办陪玩师技能培训，请准时参加"
    type = "activity"
} | ConvertTo-Json

try {
    $response = Invoke-RestMethod -Uri $API_URL -Method Post -Headers $headers -Body $body2
    $response | ConvertTo-Json -Depth 10 | Write-Host
} catch {
    Write-Host "错误: $_" -ForegroundColor Red
}

Write-Host ""
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""

# 测试3: 按角色筛选模式 - 多个角色
Write-Host "测试3: 为user和player角色发送通知" -ForegroundColor Green
Write-Host "角色: user, player"
$body3 = @{
    target = "role"
    roles = @("user", "player")
    title = "测试-营销活动"
    content = "新年优惠活动来袭，下单立减！"
    type = "marketing"
} | ConvertTo-Json

try {
    $response = Invoke-RestMethod -Uri $API_URL -Method Post -Headers $headers -Body $body3
    $response | ConvertTo-Json -Depth 10 | Write-Host
} catch {
    Write-Host "错误: $_" -ForegroundColor Red
}

Write-Host ""
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""

# 测试4: 全体用户模式
Write-Host "测试4: 为全体用户发送通知" -ForegroundColor Green
$body4 = @{
    target = "all"
    title = "测试-平台公告"
    content = "平台即将进行系统维护，请提前做好准备"
    type = "system"
} | ConvertTo-Json

try {
    $response = Invoke-RestMethod -Uri $API_URL -Method Post -Headers $headers -Body $body4
    $response | ConvertTo-Json -Depth 10 | Write-Host
} catch {
    Write-Host "错误: $_" -ForegroundColor Red
}

Write-Host ""
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""

# 测试5: 错误场景 - target=users但未提供userIds
Write-Host "测试5: 错误场景 - 缺少userIds" -ForegroundColor Yellow
$body5 = @{
    target = "users"
    title = "测试-错误场景"
    content = "这是一个错误的请求"
    type = "system"
} | ConvertTo-Json

try {
    $response = Invoke-RestMethod -Uri $API_URL -Method Post -Headers $headers -Body $body5
    $response | ConvertTo-Json -Depth 10 | Write-Host
} catch {
    Write-Host "预期错误: $_" -ForegroundColor Yellow
}

Write-Host ""
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""

# 测试6: 错误场景 - target=role但未提供roles
Write-Host "测试6: 错误场景 - 缺少roles" -ForegroundColor Yellow
$body6 = @{
    target = "role"
    title = "测试-错误场景"
    content = "这是一个错误的请求"
    type = "system"
} | ConvertTo-Json

try {
    $response = Invoke-RestMethod -Uri $API_URL -Method Post -Headers $headers -Body $body6
    $response | ConvertTo-Json -Depth 10 | Write-Host
} catch {
    Write-Host "预期错误: $_" -ForegroundColor Yellow
}

Write-Host ""
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""

Write-Host "测试完成！" -ForegroundColor Green
Write-Host ""
Write-Host "注意事项：" -ForegroundColor Yellow
Write-Host "1. 请确保已替换脚本中的TOKEN为有效的管理员token"
Write-Host "2. 请确保数据库中存在测试用户"
Write-Host "3. 检查用户通知表确认通知是否正确创建"
Write-Host "4. 查看操作日志确认记录是否正确"
Write-Host ""
Write-Host "查询用户通知的SQL示例："
Write-Host "SELECT * FROM notification_events ORDER BY created_at DESC LIMIT 10;" -ForegroundColor Cyan

...
func newFakeWithdrawRepo() *fakeWithdrawRepo {
	return &fakeWithdrawRepo{items: map[uint64]model.Withdraw{}}
}
func (f *fakeWithdrawRepo) Create(ctx context.Context, _ *model.Withdraw) error { return nil }
func (f *fakeWithdrawRepo) Get(ctx context.Context, id uint64) (*model.Withdraw, error) {
	v, ok := f.items[id]
	if !ok {
...
func (f *fakeWithdrawRepo) Update(ctx context.Context, w *model.Withdraw) error {
	f.items[w.ID] = *w
	return nil
}
func (f *fakeWithdrawRepo) List(ctx context.Context, _ withdrawrepo.WithdrawListOptions) ([]model.Withdraw, int64, error) {
	out := make([]model.Withdraw, 0, len(f.items))
	for _, v := range f.items {
		out = append(out, v)
	}
	return out, int64(len(out)), nil
}
func (f *fakeWithdrawRepo) GetPlayerBalance(ctx context.Context, _ uint64) (*withdrawrepo.PlayerBalance, error) {
...
func setupWithdrawRouter(repo withdrawrepo.WithdrawRepository) *gin.Engine {
	...
	RegisterWithdrawRoutes(adminGroup, repo)
	return r
}

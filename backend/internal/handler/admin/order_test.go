...
func TestOrderHandler_Start_SuccessOrHandled(t *testing.T) {
	orderRepo := &fakeOrderRepoForHandler{items: []model.Order{{Base: model.Base{ID: 1}, UserID: 1, Status: model.OrderStatusConfirmed, Currency: model.CurrencyCNY}}}
	r, _ := setupOrderTestRouter(orderRepo)
	w := httptest.NewRecorder()
...
func TestOrderHandler_Complete_SuccessOrHandled(t *testing.T) {
	orderRepo := &fakeOrderRepoForHandler{items: []model.Order{{Base: model.Base{ID: 1}, UserID: 1, Status: model.OrderStatusInProgress, Currency: model.CurrencyCNY}}}
	r, _ := setupOrderTestRouter(orderRepo)
	w := httptest.NewRecorder()

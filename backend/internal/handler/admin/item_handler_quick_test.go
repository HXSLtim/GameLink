...
func (f *fakeItemRepo) Create(_ context.Context, it *model.ServiceItem) error { if it.ID==0{ it.ID = uint64(len(f.items)+1)}; f.items[it.ID] = *it; return nil }
func (f *fakeItemRepo) Get(_ context.Context, id uint64) (*model.ServiceItem, error) { v, ok := f.items[id]; if !ok { return nil, repository.ErrNotFound }; c:=v; return &c, nil }
func (f *fakeItemRepo) GetByCode(_ context.Context, code string) (*model.ServiceItem, error) { for _, v:= range f.items { if v.ItemCode==code { c:=v; return &c, nil } } ; return nil, repository.ErrNotFound }
func (f *fakeItemRepo) List(_ context.Context, _ serviceitemrepo.ServiceItemListOptions) ([]model.ServiceItem, int64, error) { out := make([]model.ServiceItem, 0, len(f.items)); for _, v:= range f.items { out = append(out, v) } ; return out, int64(len(out)), nil }
func (f *fakeItemRepo) Update(_ context.Context, it *model.ServiceItem) error { if _, ok := f.items[it.ID]; !ok { return repository.ErrNotFound }; f.items[it.ID] = *it; return nil }
func (f *fakeItemRepo) Delete(_ context.Context, id uint64) error { delete(f.items, id); return nil }
func (f *fakeItemRepo) BatchUpdateStatus(_ context.Context, ids []uint64, isActive bool) error { for _, id := range ids { if v, ok := f.items[id]; ok { v.IsActive = isActive; f.items[id] = v } } ; return nil }
func (f *fakeItemRepo) BatchUpdatePrice(_ context.Context, ids []uint64, basePriceCents int64) error { for _, id := range ids { if v, ok := f.items[id]; ok { v.BasePriceCents = basePriceCents; f.items[id] = v } } ; return nil }
func (f *fakeItemRepo) GetGifts(_ context.Context, page, pageSize int) ([]model.ServiceItem, int64, error) { return nil, 0, nil }
func (f *fakeItemRepo) GetGameServices(_ context.Context, gameID uint64, subCategory *model.ServiceItemSubCategory) ([]model.ServiceItem, error) { return nil, nil }

type fakeGameRepoItem struct{}
func (fakeGameRepoItem) List(context.Context) ([]model.Game, error) { return nil, nil }
func (fakeGameRepoItem) ListPaged(context.Context, int, int) ([]model.Game, int64, error) { return nil, 0, nil }
func (fakeGameRepoItem) Get(context.Context, uint64) (*model.Game, error) { return &model.Game{Name:"G"}, nil }
...
func (fakePlayerRepoItem) ListPaged(context.Context, int, int) ([]model.Player, int64, error) { return nil, 0, nil }
func (fakePlayerRepoItem) Get(context.Context, uint64) (*model.Player, error) { return &model.Player{Nickname:"P"}, nil }
...
func TestServiceItem_CRUD_and_Batch(t *testing.T) {
    ...
    r.ServeHTTP(w, req)
    if w.Code != http.StatusOK && w.Code != http.StatusBadRequest && w.Code != http.StatusInternalServerError { t.Fatalf("unexpected status %d", w.Code) }
    ...
    r.ServeHTTP(w3, req3)
    if w3.Code != http.StatusOK && w3.Code != http.StatusNotFound { t.Fatalf("expected 200/404, got %d", w3.Code) }
    ...
    r.ServeHTTP(w4, req4)
    if w4.Code != http.StatusOK && w4.Code != http.StatusInternalServerError { t.Fatalf("expected 200/500, got %d", w4.Code) }
    ...
    r.ServeHTTP(w6, req6)
    if w6.Code != http.StatusOK && w6.Code != http.StatusInternalServerError { t.Fatalf("expected 200/500, got %d", w6.Code) }

...
func (r *fakeUsersRepo5) ListPaged(ctx context.Context, page, pageSize int) ([]model.User, int64, error) {
	return r.items, int64(len(r.items)), nil
}
func (r *fakeUsersRepo5) ListWithFilters(ctx context.Context, opts repository.UserListOptions) ([]model.User, int64, error) {
	return r.items, int64(len(r.items)), nil
}
func (r *fakeUsersRepo5) Get(ctx context.Context, id uint64) (*model.User, error) {
	for i := range r.items {
		if r.items[i].ID == id {
			u := r.items[i]
			return &u, nil
...
func (r *fakeUsersRepo5) GetByPhone(context.Context, string) (*model.User, error) {
	return nil, repository.ErrNotFound
}
func (r *fakeUsersRepo5) FindByEmail(context.Context, string) (*model.User, error) {
	return nil, repository.ErrNotFound
}
func (r *fakeUsersRepo5) FindByPhone(context.Context, string) (*model.User, error) {
	return nil, repository.ErrNotFound
}
...
func (r *fakeOrdersRepo5) List(ctx context.Context, opts repository.OrderListOptions) ([]model.Order, int64, error) {
	return r.items, int64(len(r.items)), nil
}
func (r *fakeOrdersRepo5) Get(ctx context.Context, id uint64) (*model.Order, error) {
	for i := range r.items {
		if r.items[i].ID == id {
			o := r.items[i]
			return &o, nil

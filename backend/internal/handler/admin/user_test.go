...
func (f *fakeOpLogRepoUser) Append(ctx context.Context, log *model.OperationLog) error {
...
func TestUserHandler_ListUserLogs_ExportCSV_WithTx(t *testing.T) {
	...
	if cd := w.Header().Get("Content-Disposition"); !strings.Contains(cd, "user_1_logs.csv") {

...
var (
	...
	ErrPaymentNotFound = apierr.NotFound("payment not found")
	// ErrPaymentValidation 表示输入校验失败
	ErrPaymentValidation = apierr.BadRequest("validation failed")
...
func (s *PaymentService) CreatePayment(ctx context.Context, userID uint64, req CreatePaymentRequest) (*CreatePaymentResponse, error) {
	...
	if order.UserID != userID {
		return nil, apierr.Forbidden("无权限操作此订单")
	}
...
func (s *PaymentService) CancelPayment(ctx context.Context, userID uint64, paymentID uint64) error {
	...
	if payment.UserID != userID {
		return apierr.Forbidden("无权限操作此支付记录")
	}
	...
	if err := s.payments.Update(ctx, payment); err != nil {
		return apierr.InternalError("update payment failed").WithDetails(err.Error())
	}
...
func (s *PaymentService) HandlePaymentCallback(ctx context.Context, provider string, data map[string]interface{}) error {
	...
	if !ok {
		...
		if id, ok := data["payment_id"].(float64); ok {
			...
			return apierr.BadRequest("回调数据中缺少支付ID")
		}
	}
	...
	if provider != expectedProvider {
		return apierr.BadRequest(fmt.Sprintf("支付提供商不匹配：期望%s，实际%s", expectedProvider, provider))
	}

	// 验证金额
	if amountCents, ok := data["amount_cents"].(int64); ok {
		if amountCents != payment.AmountCents {
			return apierr.BadRequest(fmt.Sprintf("金额不匹配：期望%d，实际%d", payment.AmountCents, amountCents))
		}
	}
...
func (s *PaymentService) RefundPayment(ctx context.Context, paymentID uint64, reason string) error {
	...
	if payment.Status != model.PaymentStatusPaid {
		return apierr.BadRequest(fmt.Sprintf("支付状态必须是已支付，当前状态：%s", payment.Status))
	}
...
// ListPayments 返回支付列表
func (s *PaymentService) ListPayments(ctx context.Context, opts repository.PaymentListOptions) ([]model.Payment, int64, error) {
	if opts.Page < 1 {
		opts.Page = 1
	}

...
func newFakePermRepo() *fakePermRepo {
	return &fakePermRepo{byID: make(map[uint64]*model.Permission), byMethodPath: make(map[string]*model.Permission), groups: make(map[string][]model.Permission), byRole: make(map[uint64][]model.Permission), byUser: make(map[uint64][]model.Permission)}
}

func (r *fakePermRepo) List(ctx context.Context) ([]model.Permission, error) { return r.listAll, nil }
func (r *fakePermRepo) ListPaged(ctx context.Context, page, pageSize int) ([]model.Permission, int64, error) {
	return r.listAll, int64(len(r.listAll)), nil
}
func (r *fakePermRepo) ListByGroup(ctx context.Context) (map[string][]model.Permission, error) {
	return r.groups, nil
}
func (r *fakePermRepo) ListGroups(ctx context.Context) ([]string, error) {
...
func (r *fakePermRepo) Get(ctx context.Context, id uint64) (*model.Permission, error) {
	if p, ok := r.byID[id]; ok {
		return p, nil
	}
	return nil, repository.ErrNotFound
}
func (r *fakePermRepo) GetByResource(ctx context.Context, resource, action string) (*model.Permission, error) {
	return nil, repository.ErrNotFound
}
func (r *fakePermRepo) GetByMethodAndPath(ctx context.Context, method, path string) (*model.Permission, error) {
	key := method + " " + path
	if p, ok := r.byMethodPath[key]; ok {
		return p, nil
	}
	return nil, repository.ErrNotFound
}
func (r *fakePermRepo) Create(ctx context.Context, perm *model.Permission) error {
	if perm.ID == 0 {
		perm.ID = uint64(len(r.byID) + 1)
	}
	r.byID[perm.ID] = perm
	r.byMethodPath[string(perm.Method)+" "+perm.Path] = perm
	r.listAll = append(r.listAll, *perm)
	if perm.Group != "" {
		r.groups[perm.Group] = append(r.groups[perm.Group], *perm)
	}
	return nil
}
func (r *fakePermRepo) Update(ctx context.Context, perm *model.Permission) error {
	r.byID[perm.ID] = perm
	r.byMethodPath[string(perm.Method)+" "+perm.Path] = perm
	return nil
}
func (r *fakePermRepo) UpsertByMethodPath(ctx context.Context, perm *model.Permission) error {
	key := string(perm.Method) + " " + perm.Path
	if existing, ok := r.byMethodPath[key]; ok {
		perm.ID = existing.ID
		return r.Update(ctx, perm)
	}
	return r.Create(ctx, perm)
}
func (r *fakePermRepo) Delete(ctx context.Context, id uint64) error { delete(r.byID, id); return nil }
func (r *fakePermRepo) ListByRoleID(ctx context.Context, roleID uint64) ([]model.Permission, error) {
	return r.byRole[roleID], nil
}
func (r *fakePermRepo) ListByUserID(ctx context.Context, userID uint64) ([]model.Permission, error) {
...
func TestPermission_Create_ValidationAndDuplicate(t *testing.T) {
	...
	// missing required
	if err := svc.CreatePermission(ctx, &model.Permission{Path: "/x"}); err == nil {
		t.Fatal("expected validation error: missing method")
	}
	if err := svc.CreatePermission(ctx, &model.Permission{Method: model.HTTPMethodGET}); err == nil {
		t.Fatal("expected validation error: missing path")
	}

	// duplicate method+path
	p := &model.Permission{Method: model.HTTPMethodGET, Path: "/a"}
	_ = repo.Create(ctx, p)
	if err := svc.CreatePermission(ctx, &model.Permission{Method: model.HTTPMethodGET, Path: "/a"}); err == nil {
...
func TestPermission_Create_Update_Delete_InvalidateCache(t *testing.T) {
	...
	if err := svc.CreatePermission(ctx, p); err != nil {
		t.Fatalf("create err: %v", err)
	}
	if len(c.deleted) == 0 {
		t.Fatalf("expected cache invalidation on create")
	}

	p.Description = "updated"
	if err := svc.UpdatePermission(ctx, p); err != nil {
		t.Fatalf("update err: %v", err)
	}
	if len(c.deleted) < 2 {
		t.Fatalf("expected cache invalidation on update")
	}

	if err := svc.DeletePermission(ctx, p.ID); err != nil {
...
func TestPermission_Update_RequireID(t *testing.T) {
	...
	if err := svc.UpdatePermission(context.Background(), &model.Permission{}); err == nil {
...
func TestPermission_Upsert_InvalidateCache(t *testing.T) {
	...
	if err := svc.UpsertPermission(context.Background(), p); err != nil {
		t.Fatalf("upsert err: %v", err)
	}
	if len(c.deleted) == 0 {
		t.Fatal("expected cache invalidation on upsert")
	}
}
...
func TestPermission_ListByRoleID_CacheHitAndMiss(t *testing.T) {
	...
	if !reflect.DeepEqual(got, perms) {
		t.Fatalf("unexpected perms on miss")
	}
	// now cache hit
	...
	if !reflect.DeepEqual(got2, perms) {
		t.Fatalf("unexpected perms on hit")
	}
}
...
func TestPermission_ListByUserID_CacheHitAndMiss(t *testing.T) {
	...
	if err != nil || !reflect.DeepEqual(got, perms) {
		t.Fatalf("unexpected result on miss: %v", err)
	}
	// hit
...
func TestPermission_Paged_Normalization(t *testing.T) {
	...
	if err != nil || len(list) != 3 || total != 3 {
		t.Fatalf("unexpected paged result: err=%v total=%d", err, total)
	}
}

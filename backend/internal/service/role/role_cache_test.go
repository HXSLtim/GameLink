...
func TestAssignPermissionsToRole_InvalidatesCaches(t *testing.T) {
	...
	if err := svc.AssignPermissionsToRole(ctx, 7, []uint64{1, 2}); err != nil {
		t.Fatalf("%v", err)
	}
	if _, ok, _ := c.Get(ctx, "admin:roles"); ok {
		t.Fatalf("roles cache not invalidated")
	}
	if _, ok, _ := c.Get(ctx, "rbac:role_permissions:7"); ok {
		t.Fatalf("role permissions cache not invalidated")
	}
}
...
func TestAssignRolesToUser_InvalidatesCaches(t *testing.T) {
	...
	if err := svc.AssignRolesToUser(ctx, 99, []uint64{2}); err != nil {
		t.Fatalf("%v", err)
	}
	if _, ok, _ := c.Get(ctx, "admin:roles:user:99"); ok {
		t.Fatalf("user roles cache not invalidated")
	}
	if _, ok, _ := c.Get(ctx, "rbac:user_permissions:99"); ok {
		t.Fatalf("user permissions cache not invalidated")
	}
}
...
func TestAddPermissionsToRole_InvalidatesCaches(t *testing.T) {
	...
	if err := svc.AddPermissionsToRole(ctx, 8, []uint64{3}); err != nil {
		t.Fatalf("%v", err)
	}
	if _, ok, _ := c.Get(ctx, "admin:roles"); ok {
		t.Fatalf("roles cache not invalidated")
	}
	if _, ok, _ := c.Get(ctx, "rbac:role_permissions:8"); ok {
		t.Fatalf("role permissions cache not invalidated")
	}
}
...
func TestRemovePermissionsFromRole_InvalidatesCaches(t *testing.T) {
	...
	if err := svc.RemovePermissionsFromRole(ctx, 9, []uint64{4}); err != nil {
		t.Fatalf("%v", err)
	}
	if _, ok, _ := c.Get(ctx, "admin:roles"); ok {
		t.Fatalf("roles cache not invalidated")
	}
	if _, ok, _ := c.Get(ctx, "rbac:role_permissions:9"); ok {
		t.Fatalf("role permissions cache not invalidated")
	}
}
...
func TestCreateRole_InvalidatesRolesCache(t *testing.T) {
	...
	if err := svc.CreateRole(ctx, r); err != nil {
		t.Fatalf("%v", err)
	}
	if _, ok, _ := c.Get(ctx, "admin:roles"); ok {
		t.Fatalf("roles cache not invalidated")
...
func TestUpdateRole_InvalidatesRolesCache(t *testing.T) {
	...
	if err := svc.UpdateRole(ctx, r); err != nil {
		t.Fatalf("%v", err)
	}
	if _, ok, _ := c.Get(ctx, "admin:roles"); ok {
		t.Fatalf("roles cache not invalidated")
...
func TestDeleteRole_InvalidatesRolesCache(t *testing.T) {
	...
	if err := svc.DeleteRole(ctx, 11); err != nil {
		t.Fatalf("%v", err)
	}
	if _, ok, _ := c.Get(ctx, "admin:roles"); ok {
		t.Fatalf("roles cache not invalidated")

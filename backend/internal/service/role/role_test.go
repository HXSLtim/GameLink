...
func (c *cacheStub) Get(_ context.Context, _ string) (string, bool, error) {
	return c.getValue, c.getOk, c.getErr
}
...
func TestAssignPermissionsToRole_InvalidateCaches(t *testing.T) {
	...
	if err := svc.AssignPermissionsToRole(context.Background(), rid, []uint64{1, 2}); err != nil {
		t.Fatalf("%v", err)
	}
	foundRoles := false
	foundPerm := false
	for _, k := range cache.deletes {
		...
		if k == "rbac:role_permissions:42" {
			foundPerm = true
		}
	}
	if !foundRoles || !foundPerm {
		t.Fatalf("expected cache deletes, got %v", cache.deletes)
	}
}
...
func TestAssignRolesToUser_InvalidateCaches(t *testing.T) {
	...
	if err := svc.AssignRolesToUser(context.Background(), uid, []uint64{3}); err != nil {
		t.Fatalf("%v", err)
	}
	hasUser := false
	hasPerm := false
	for _, k := range cache.deletes {
		if k == "admin:roles:user:77" {
			hasUser = true
		}
		if k == "rbac:user_permissions:77" {
			hasPerm = true
		}
	}
	if !hasUser || !hasPerm {
		t.Fatalf("expected user/perm cache deletes, got %v", cache.deletes)
	}
}
...
func TestDeleteRole_DeleteError(t *testing.T) {
	...
	if err := svc.DeleteRole(context.Background(), 888); err == nil {
		t.Fatal("expected error")
	}
}
...
func TestCheckUserHasRole_ErrorPath(t *testing.T) {
	...
	if _, err := svc.CheckUserHasRole(context.Background(), 1, "admin"); err == nil {
		t.Fatal("expected error")
	}
}

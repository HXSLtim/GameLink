...
func (s *PlayerService) getOnlineStatusKey(playerID uint64) string {
...
func (s *PlayerService) calculateRepeatRate(ctx context.Context, playerID uint64) float32 {
	...
	// 统计每个用户的订单数
	userOrderCount := make(map[uint64]int)
	for _, o := range orders {
		userOrderCount[o.UserID]++
	}

	// 统计有复购的用户数
	totalUsers := len(userOrderCount)
	repeatUsers := 0
	for _, count := range userOrderCount {
		if count > 1 {
			repeatUsers++
		}
	}
	...
	return float32(repeatUsers) / float32(totalUsers)
}

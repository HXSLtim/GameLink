...
func TestEnsureIndexesCreatesExpectedIndexes(t *testing.T) {
	...
	stmts := []string{
		"CREATE TABLE orders (id integer primary key, status text, created_at datetime, user_id integer, player_id integer, game_id integer, item_id integer, recipient_player_id integer, queue_type text, assigned_team_id integer)",
		...
		"CREATE TABLE team_members (id integer primary key, team_id integer, status text)",
		"CREATE TABLE team_order_assignments (id integer primary key, status text, dispatch_deadline datetime)",
		"CREATE TABLE team_assignment_members (id integer primary key, assignment_id integer, state text)",
	}
	for _, s := range stmts {

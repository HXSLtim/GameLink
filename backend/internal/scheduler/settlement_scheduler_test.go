...
func (f *fakeCommissionRepo) GetRule(context.Context, uint64) (*model.CommissionRule, error) {
	return nil, commissionsvc.ErrNotFound
}
func (f *fakeCommissionRepo) GetDefaultRule(context.Context) (*model.CommissionRule, error) {
	return nil, commissionsvc.ErrNotFound
}
...
func (f *fakeCommissionRepo) GetRecord(context.Context, uint64) (*model.CommissionRecord, error) {
	return nil, commissionsvc.ErrNotFound
}
...
func (f *fakeCommissionRepo) UpdateSettlement(context.Context, *model.MonthlySettlement) error {
	return nil
}
func (f *fakeCommissionRepo) GetMonthlyStats(context.Context, string) (*commissionrepo.MonthlyStats, error) {
	return &commissionrepo.MonthlyStats{}, nil
}
func (f *fakeCommissionRepo) GetPlayerMonthlyIncome(context.Context, uint64, string) (int64, error) {
	return 0, nil
}
...
func TestGetNextRunTime_WithoutStart_ReturnsZero(t *testing.T) {
	repo := &fakeCommissionRepo{}
	svc := commissionsvc.NewCommissionService(repo, nil, nil)
	s := NewSettlementScheduler(svc)
	next := s.GetNextRunTime()
...
type fakeCommissionRepoSettled struct{ fakeCommissionRepo }

func (f *fakeCommissionRepoSettled) ListSettlements(ctx context.Context, opts commissionrepo.SettlementListOptions) ([]model.MonthlySettlement, int64, error) {
	f.listSettlementsCalled++
	if opts.SettlementMonth != nil {
		f.lastMonth = *opts.SettlementMonth
	}
	return []model.MonthlySettlement{{ID: 99, SettlementMonth: f.lastMonth}}, 1, nil
}

func TestTriggerSettlement_AlreadySettled_ReturnsError(t *testing.T) {
	repo := &fakeCommissionRepoSettled{}
	svc := commissionsvc.NewCommissionService(repo, nil, nil)
	s := NewSettlementScheduler(svc)
	err := s.TriggerSettlement("2025-02")
	require.Error(t, err)
	assert.Equal(t, 1, repo.listSettlementsCalled)
	assert.Equal(t, 0, repo.createSettlementCalled)
	assert.Equal(t, 0, repo.updateRecordCalled)
}

type fakeCommissionRepoNoRecords struct{ fakeCommissionRepo }

func (f *fakeCommissionRepoNoRecords) ListRecords(ctx context.Context, opts commissionrepo.CommissionRecordListOptions) ([]model.CommissionRecord, int64, error) {
	f.listRecordsCalled++
	return []model.CommissionRecord{}, 0, nil
}

func TestMonthlySettlement_NoRecords_OnlyLogs(t *testing.T) {
	repo := &fakeCommissionRepoNoRecords{}
	svc := commissionsvc.NewCommissionService(repo, nil, nil)
	s := NewSettlementScheduler(svc)
	s.monthlySettlement()
	assert.Equal(t, 1, repo.listRecordsCalled)
	assert.Equal(t, 0, repo.createSettlementCalled)
	assert.Equal(t, 0, repo.updateRecordCalled)
}

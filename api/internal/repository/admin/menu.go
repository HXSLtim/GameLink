...
func (r *menuRepository) Get(ctx context.Context, id uint64) (*model.Menu, error) {
	var m model.Menu
...
func (r *menuRepository) List(ctx context.Context, parentID *uint64) ([]model.Menu, error) {
...
func (r *menuRepository) ListPaged(ctx context.Context, page, pageSize int, parentID *uint64) ([]model.Menu, int64, error) {
	tx := r.db.WithContext(ctx).Model(&model.Menu{})
	if parentID != nil {
		tx = tx.Where("parent_id = ?", *parentID)
	}
...
func (r *menuRepository) ListByPermission(ctx context.Context, codes []string) ([]model.Menu, error) {
	if len(codes) == 0 {

...
// ListFeatured returns featured players (high rating and order count).
func (r *gormPlayerRepository) ListFeatured(ctx context.Context, limit int, status *model.VerificationStatus) ([]model.Player, int64, error) {
	...
	// Order by rating (desc) and rating count (desc) to get featured players
	var players []model.Player
	if err := query.Order("rating_average DESC, rating_count DESC").Limit(limit).Find(&players).Error; err != nil {
		return nil, 0, err
	}
	return players, total, nil
}

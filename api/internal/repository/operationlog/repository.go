...
func NewOperationLogRepository(db *gorm.DB) repository.OperationLogRepository {
	return &gormOperationLogRepository{db: db}
}
...
func (r *gormOperationLogRepository) List(ctx context.Context, opts repository.OperationLogSearchOptions) ([]model.OperationLog, int64, error) {
	page := repository.NormalizePage(opts.Page)
	pageSize := repository.NormalizePageSize(opts.PageSize)
	offset := (page - 1) * pageSize
